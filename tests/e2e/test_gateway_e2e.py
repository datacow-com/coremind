...
def test_env_versions_and_credentials():
    token = _get_token()
    headers = _auth_headers(token)
    r = httpx.post(f"{API}/models/environments/dev/versions", headers=headers, json={"config": {"llm": {"default": "qwen-plus"}}}, timeout=20)
    assert r.status_code in (200, 201)
    ...
    r = httpx.post(f"{API}/models/environments/dev/versions/{latest}/apply", headers=headers, params={"dry_run": True}, timeout=10)
    assert r.status_code == 200
    r = httpx.post(f"{API}/models/environments/dev/versions/{latest}/apply", headers=headers, params={"dry_run": False}, timeout=10)
    assert r.status_code == 200
...
def test_credentials_and_ingest():
    ...
    assert provider_id
    cred_payload = {"auth_type": "api_key", "key_name": "dashscope", "rate_limit_rps": 10, "quota_limit": 1000, "quota_window": "daily"}
    r = httpx.post(f"{API}/models/{provider_id}/credentials/dev", headers=headers, json=cred_payload, timeout=20)
    assert r.status_code in (200, 201)
