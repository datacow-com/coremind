...
export default function UsageDashboard() {
  ...
  const load = async () => {
    try {
      const u = await fetch("/api/metrics/usage").then((r) =>
        r.ok ? r.json() : Promise.reject("usage"),
      );
      setUsage(u.usage || {});
      setSummary(u.summary || {});
    } catch {}
    try {
      const t = await fetch("/api/alerts/thresholds").then((r) =>
        r.ok ? r.json() : Promise.reject("thresholds"),
      );
            ...
            <div key={day} className="mb-4">
              ...
              {summary[day] && (
                <div className="text-xs text-gray-600 mt-1">
                  Summary: calls {summary[day].calls} · tokens{" "}
                  {summary[day].tokens} · duration {summary[day].duration_ms} ms
                </div>
              )}
              <table className="w-full text-sm mt-2">
                <thead>
                  <tr className="text-left border-b">
                    ...
                    <th className="py-1">Tokens In</th>
                    <th className="py-1">Tokens Out</th>
                    <th className="py-1">Duration (ms)</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="space-y-2">
            <div>
              ...
              <input
                ...
                type="number"
                value={thresholds.max_tokens_per_day ?? ""}
                onChange={(e) =>
                  setThresholds({
                    ...thresholds,
                    max_tokens_per_day: Number(e.target.value),
                  })
                }
              />
            </div>
            <div>
              ...
              <input
                className="border rounded px-2 py-1 w-full"
                type="number"
                value={thresholds.max_calls_per_day ?? ""}
                onChange={(e) =>
                  setThresholds({
                    ...thresholds,
            ...
            <div>
              ...
              <input
                ...
                type="number"
                step="0.000001"
                value={thresholds.max_cost_per_day ?? ""}
                onChange={(e) =>
                  setThresholds({
                    ...thresholds,
                    max_cost_per_day: Number(e.target.value),
                  })
                }
              />
            </div>
            <div>
              ...
              <input
                className="border rounded px-2 py-1 w-full"
                type="text"
                value={thresholds.webhook_url ?? ""}
                onChange={(e) =>
                  setThresholds({ ...thresholds, webhook_url: e.target.value })
                }
              />
            </div>
            <button
