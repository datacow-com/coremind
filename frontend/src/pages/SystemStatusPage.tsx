...
export default function SystemStatusPage() {
  ...
  const Badge = ({ ok, label }: { ok: boolean; label: string }) => (
    <span
      className={`px-2 py-1 rounded text-xs ${ok ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
    >
      {label}: {ok ? "OK" : "DOWN"}
    </span>
  );
        ...
        {webStatus && (
          <div className="mb-3 text-sm">
            <div className="mb-1">
              ...
              <span className="font-medium">{webStatus.current_provider}</span>
            </div>
            <div className="space-x-2">
              {Object.keys(webStatus.providers).map((name) => (
                <button
                  key={name}
                  onClick={() => selectWebProvider(name)}
                  className={`px-2 py-1 text-xs rounded ${webStatus.providers[name].configured ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-700"}`}
                >
                  {name}
                </button>
              ))}
              ...
              {validations.length === 0 && (
                <tr>
                  <td className="px-3 py-2 text-gray-500" colSpan={3}>
                    No data. Demo Login then Fetch Providers.
                  </td>
                </tr>
              )}
            </tbody>
