...
class OSSBlobStorage(BlobStorage):
    ...
    async def stream_get(self, path: str, chunk_size: int = 1024 * 1024):
        # Stream by chunk via thread to avoid blocking event loop
        import asyncio
...
class MinIOBlobStorage(BlobStorage):
    ...
    async def put(self, path: str, content: bytes) -> str:
        ...
        await asyncio.to_thread(
            self.client.put_object, self.bucket_name, path, stream, length=len(content)
        )
...
_BLOB_STORE: BlobStorage | None = None
...
def get_blob_store() -> BlobStorage:
    ...
    store_type = (cfg.get("blob_type") or os.environ.get("BLOB_STORE_TYPE") or "local").lower()
